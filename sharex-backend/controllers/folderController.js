...
// Obtener ruta actual
export const currentPathController = (req, res) => {
  res.send({
    message: "Ruta actual",
    ...
    const fileList = files
      .filter((file) => {
        const systemFiles = [
          ...
          "partition_identifier_bc_new.platform",
        ];
        return (
...
export const createFolderController = async (req, res) => {
  ...
  if (!folderName)
    return res
      .status(400)
      .send({ error: "El nombre de la carpeta es requerido." });
  ...
  try {
    ...
    res
      .status(500)
      .send({ error: `Error al crear la carpeta: ${err.message}` });
  }
};
...
export const deleteFolderController = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .send({ error: `Error al eliminar la carpeta: ${err.message}` });
  }
};
...
export const renameFolderController = async (req, res) => {
  ...
  if (!oldFolderName || !newFolderName)
    return res
      .status(400)
      .send({ error: "Se requieren los nombres de la carpeta vieja y nueva." });
  ...
  try {
    ...
    res
      .status(500)
      .send({ error: `Error al renombrar la carpeta: ${err.message}` });
  }
};

// Navegar a subcarpeta
export const navigateController = async (req, res) => {
  const { dir } = req.query;
  if (!dir)
    return res
      .status(400)
      .send({ error: "Se requiere un parámetro de carpeta." });
  ...
  try {
    ...
    if (!stats.isDirectory())
      return res
        .status(400)
        .send({ error: "El directorio no existe o no es válido." });
    currentPath = newPath;
...
// Navegar hacia atrás
export const backController = (req, res) => {
